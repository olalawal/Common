   ...
   public  class ApiKeyServiceModule : NinjectModule
    {
        public override void Load()
        {
            ...
            this.Bind<IUnitOfWork>().ToMethod(ctx => ctx.Kernel.Get<ApiKeyContext>()).When(t => t.IsInjectingToRepositoryDataSourceOfNamespace("Nmedia.Services.Authorization.ApiKeyService"));
            // this.Unbind(typeof(DbContext));
            this.Bind<DbContext>().ToMethod(ctx => ctx.Kernel.Get<ApiKeyContext>()).When(t => t.IsInjectingToRepositoryDataSourceOfNamespace("Nmedia.Services.Authorization.ApiKeyService"));
